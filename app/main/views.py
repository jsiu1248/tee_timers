...
def leaderboard_18holes():
    ...
    return render_template('leaderboard.html', title='18 Hole Leaderboard', sorted_user_points=sorted_user_points)
...
def golf_log():
    ...
    # Pagination of the posts for all users
    pagination = \
        GolfLog.query.join(Action, Satisfaction).order_by(GolfLog.timestamp.desc()).paginate(
            page = page,
            per_page = current_app.config['POSTS_PER_PAGE'],
            error_out = False)
    # Convert to list
